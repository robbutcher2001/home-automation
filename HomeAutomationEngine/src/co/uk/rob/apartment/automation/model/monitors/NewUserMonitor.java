			...
			if (UserManager.readUsers()) {
				...
				for (int index = 0; index < users.size(); index++) {
					if (!users.get(index).isActivationTextSent()) {
						...
						SMSHelper.sendSMS(users.get(index).getMobileNumber(), users.get(index).getFirstName() + ", your user has now been activated. You can control Rob's flat here: https://www.robsflat.co.uk/");
						toUpdate = index;
					}
				}
